  ...
  async function setupSearch() {
    try {
      ...
      pagefind = await import(/* @vite-ignore */
        config.BASE + "/pagefind/pagefind.js"
      );
    } catch (error) {
  ...
  >
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto p-4 pt-20 md:p-20">
      <button
        ...
        class="fixed inset-0 bg-base-500/10 dark:bg-base-950/0 transition-opacity z-0 cursor-default backdrop-blur-sm"
      >
        <div class="sr-only">hide search</div>
      </button>

      <div
        class="mx-auto max-w-xl transform divide-y divide-base-100 dark:divide-white/10 overflow-hidden rounded-3xl bg-white dark:bg-base-900 shadow-2xl ring-1 ring-black dark:ring-white/20 ring-opacity-5 transition-all"
      >
